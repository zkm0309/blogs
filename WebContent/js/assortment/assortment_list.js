$(function(){
	...
	$("#add").click(function(){
		...
		if(nodes.length == 1){
			...
			layer.msg('请选择一个上级分类!',{icon:0,time:1000});
		}
	});
	...
	$("#batchDel").click(function(){
		...
		if(nodes.length == 0){
			layer.msg('请选择一个分类!',{icon:0,time:1000});
			return;
		}
		var ids = "";
			...
			$.ajax({
				...
				success : function(data) {
					if(data.success){
						...
						layer.msg('删除区域中有子分类,请先删除分类信息!',{icon:2,time:2000});
					}
				}
			});
		});
	...
	$("#form-admin-add").validate({
		...
		submitHandler:function(form){
			var url = $("#ctx").val()+"/assortment/add.do";
			if($("input[name=id]").val()!=""){
				url = $("#ctx").val()+"/assortment/update.do";
			}
			$(form).ajaxSubmit({
				type: 'post',
				url:  url,
...
function click(event,treeId, treeNode){
	...
	$.ajax({
		type : "POST",
		url : $("#ctx").val()+"/assortment/getById.do",
		data : {id:id},
		async : false,
		dataType : "json",
		success : function(data) {
